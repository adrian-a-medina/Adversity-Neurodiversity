...
p_load(dplyr, tidyr, tidyverse, knitr, ggplot2, Hmisc, broom, stats, kableExtra, webshot2, sjPlot, margins, gtsummary, ggstats, broom.helpers, patchwork, sjlabelled, sjmisc, vtable, DiagrammeR, survey)
...
neurodivergent_data <- neurodivergent_data %>%
  mutate(
    # Recode the four FRI items: assign 1 point for "All of the time" (response code 1)
    talkabout_score = ifelse(talkabout == 1, 1, 0),
    wktosolve_score = ifelse(wktosolve == 1, 1, 0),
    strengths_score = ifelse(strengths == 1, 1, 0),
    hopeful_score = ifelse(hopeful == 1, 1, 0),
    
    # Recode the two additional items: assign 1 point for "Very well" (response code 1)
    k8q21_score = ifelse(k8q21 == 1, 1, 0),
    k8q30_score = ifelse(k8q30 == 1, 1, 0),
    
    # Calculate the total FRCI score by summing all six items
    FRCI = talkabout_score + wktosolve_score + strengths_score + hopeful_score + k8q21_score + k8q30_score,
