...
p_load(dplyr, tidyr, tidyverse, knitr, ggplot2, Hmisc, broom, stats, kableExtra, webshot2, sjPlot, margins, gtsummary, ggstats, broom.helpers, patchwork, sjlabelled, sjmisc, vtable)
...
knitr::kable(prevalence_table, "html", caption = "Prevalence of Neurodivergent Conditions") %>%
...
cross_tabs <- neurodivergent_data %>%
  select(Anxiety, Depression, Behavioral_Issues, CFI_dich, ace_counts) %>%
  pivot_longer(cols = c(Anxiety, Depression, Behavioral_Issues, CFI_dich), names_to = "Condition", values_to = "Status") %>%
  filter(!is.na(Status), !is.na(ace_counts), !(Condition %in% c("Anxiety", "Depression", "Behavioral_Issues") & Status == "No")) %>%  # Filter out NA values and "No" responses
