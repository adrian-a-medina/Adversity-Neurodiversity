...
p_load(dplyr, tidyr, tidyverse, knitr, ggplot2, Hmisc, broom, stats, kableExtra, webshot2, sjPlot, margins, gtsummary, ggstats, broom.helpers, patchwork, sjlabelled, sjmisc, vtable, DiagrammeR, survey, sandwich, lmtest)
...
neurodivergent_data <- Data2_2016to2022 %>%
  select(year, fpl, fpl_mean, sc_age_years, sc_hispanic_r, sc_race_r, sc_sex, higrade_tvis, k2q35a, k2q35c, k2q38a,
         ...
         ace1, ace3, ace4, ace5, ace6, ace7, ace8, ace9, ace10, ace11, ace12, k6q71_r, k7q85_r, k7q84_r, talkabout,
         wktosolve, strengths, hopeful, k8q21, k8q30, fwc) %>%
...
# Define the path to the dataset
data_path <- "~/Downloads/nsch_2023e_topical.csv"  
...
neurodivergent_data_16to22 <- read.csv("neurodivergent_data_2016to2022.csv")
...
neurodivergent_data <- neurodivergent_data %>%
  mutate(
    # Recode the four FRI items: assign 1 point for "All of the time" (response code 1), handling missing values
    talkabout_score = ifelse(is.na(talkabout), NA, ifelse(talkabout == 1, 1, 0)),
    wktosolve_score = ifelse(is.na(wktosolve), NA, ifelse(wktosolve == 1, 1, 0)),
    strengths_score = ifelse(is.na(strengths), NA, ifelse(strengths == 1, 1, 0)),
    hopeful_score = ifelse(is.na(hopeful), NA, ifelse(hopeful == 1, 1, 0)),
    
    # Recode the two additional items: assign 1 point for "Very well" (response code 1), handling missing values
    k8q21_score = ifelse(is.na(k8q21), NA, ifelse(k8q21 == 1, 1, 0)),
    k8q30_score = ifelse(is.na(k8q30), NA, ifelse(k8q30 == 1, 1, 0)),
    
    # Calculate the total FRCI score by summing all six items, ignoring missing values
    FRCI = rowSums(across(c(talkabout_score, wktosolve_score, strengths_score, hopeful_score, k8q21_score, k8q30_score)), na.rm = TRUE),
    
    # Create a categorical version of FRCI
    FRCI_cat = factor(
      case_when(
        FRCI <= 1 ~ "0-1",
        FRCI <= 3 ~ "2-3",
...
2.  Part of this includes the calculating of the Childhood Flourishing Index (CFI) and the Family Resilience and Connection Index (FRCI) based on Bethell et al. (2019) criteria
...
Display the number of subjects in each Family Resilience and Connection Index (FRCI) score category. Visualize the distribution of children by FRCI score using bar plots for both the raw FRCI scores and the categorized FRCI variable.

```{r family-resilience-connection-index, message=FALSE, warning=FALSE}
# Bar plot of FRCI counts categories
ggplot(neurodivergent_data, aes(x = factor(FRCI), fill = factor(FRCI))) +
  geom_bar(show.legend = FALSE) +
  geom_text(stat = 'count', aes(label = ..count..), position = position_stack(vjust = 0.5), color = "white") +
  labs(x = "Family Resilience and Connection Index (FRCI) Score",
       y = "Number of Children",
       fill = "FRCI Score",
...
# Bar plot of FRCI_cat counts categories
ggplot(neurodivergent_data, aes(x = FRCI_cat)) +
  geom_bar(aes(fill = FRCI_cat), show.legend = FALSE) +
  geom_text(stat = 'count', aes(label = ..count..), position = position_stack(vjust = 0.5), color = "white") +
  labs(title = "Distribution of FRCI Categorical Variable",
...
model_anx_mod <- svyglm(Anxiety ~ ace_counts * CFI_dich + sc_race_cat + sc_sex_cat + fpl_cat + highgrade_tvis_cat + sc_age_years, 
                        design = design, family = quasibinomial())

# Perform the Breusch-Pagan test for heteroscedasticity
lmtest::bptest(model_anx_mod)
...
plot(cooks_anx, type = "h", main = "Cook's Distance for Anxiety Model",
     ylab = "Cook's Distance", xlab = "Observation Index")
abline(h = 4 / length(cooks_d_dan), col = "red", lty = 2)  # Add a reference line for influential points

# Obtain robust standard errors for Anxiety model
robust_anx <- lmtest::coeftest(model_anx_mod, vcov = vcovHC(model_anx_mod, type = "HC3"))
...
model_dep_mod <- svyglm(Depression ~ ace_counts * CFI_dich + sc_race_cat + sc_sex_cat + fpl_cat + highgrade_tvis_cat + 
                        sc_age_years, 
                        design = design, family = quasibinomial())

# Perform the Breusch-Pagan test for heteroscedasticity
lmtest::bptest(model_dep_mod)

# Calculate Cook's distance for influential observations (leverage) in each model
cooks_dep <- cooks.distance(model_dep_mod)

# Plot for Depression model
plot(cooks_dep, type = "h", main = "Cook's Distance for Depression Model",
     ylab = "Cook's Distance", xlab = "Observation Index")
abline(h = 4 / length(cooks_d_sn), col = "green", lty = 2)  # Add a reference line for influential points

# Obtain robust standard errors for Depression model
robust_dep <- lmtest::coeftest(model_dep_mod, vcov = vcovHC(model_dep_mod, type = "HC3"))
...
model_beh_mod <- svyglm(Behavioral_Problems ~ ace_counts * CFI_dich + sc_race_cat + sc_sex_cat + fpl_cat + highgrade_tvis_cat + 
                        sc_age_years, 
                        design = design, family = quasibinomial())

# Perform the Breusch-Pagan test for heteroscedasticity
lmtest::bptest(model_beh_mod)

# Calculate Cook's distance for influential observations (leverage) in each model
cooks_beh <- cooks.distance(model_beh_mod)

# Plot for Behavioral Problems model
plot(cooks_beh, type = "h", main = "Cook's Distance for Behavioral Problems Model",
     ylab = "Cook's Distance", xlab = "Observation Index")
abline(h = 4 / length(cooks_d_ln), col = "blue", lty = 2)  # Add a reference line for influential points

# Obtain robust standard errors for Behavioral Problems model
robust_beh <- lmtest::coeftest(model_beh_mod, vcov = vcovHC(model_beh_mod, type = "HC3"))
...
1.  The **anxiety** model **did not** yield significant interactions, so we **do not** proceed with stratified post-hoc analyses.
2.  The **depression** model **did** yield significant interactions, so we **do** proceed with stratified post-hoc analyses.
3.  The **behavioral problems** model **did** yield significant interactions, so we **do** proceed stratified with post-hoc analyses.
