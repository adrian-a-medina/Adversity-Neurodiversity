...
tab_model(model_anx, model_dep, model_beh, show.ci = 0.95) # <1>
...
1. The `tab_model` function from the `sjPlot` package automatically exponentiates the logistic regression coefficients into Odds Ratios when displaying models with a binomial family (logistic regression).

```{r Interaction Term Models}
#| column: screen
#| out-width: 100%
#| layout-ncol: 3
...
tab_model(model_anx_mod, model_dep_mod, model_beh_mod, show.ci = 0.95, transform = NULL) # <1>
...
1. Since we included interaction terms, we want to see the raw coefficients instead. You can set `transform = NULL` in the `tab_model` function to ensure the values aren't exponetiated into Odds Ratios.
